...
@router.post("/update_task", response_model=None, status_code=status.HTTP_201_CREATED)
async def create_task(id: int, deadline, deskription: str, db: Session = Depends(get_db)) -> dict[str, any]:
    try:
        task = TaskService.create_task(db, id, deadline, deskription)
        # Создаем словарь для возврата данных
        response_data = {
